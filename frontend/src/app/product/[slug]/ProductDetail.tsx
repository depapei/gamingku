...
export const ProductDetail = () => {
  ...
  const handleVariantChange = (variantId: string, option: string) => {
    ...
    // setSelectedVariants({ [variantId]: option });
  };
  ...
  const RenderSpecifications = () => {
    return (
      <>
        <p className="text-2xl font-medium text-zinc-900 mb-4">
          Specifications
        </p>
        <ol className="mb-8">
          {Object.entries(product.specifications).map(([key, value]) => (
            <li
              key={key}
              className="text-xs text-zinc-600 leading-relaxed mb-1 flex flex-row gap-2"
            >
        ...
        <div className="w-full md:w-1/2 flex flex-col">
          ...
          >
            ...
            <div className="flex items-end space-x-3 mb-8">
              {product.discountPrice ? (
                <>
                  ...
                  <span className="text-lg text-zinc-500 line-through mb-0.5">
            ...
            <div className="text-sm text-zinc-500 flex items-center">
              <div
                className={`w-2 h-2 rounded-full mr-2 ${product.stock > 0 ? "bg-emerald-500" : "bg-red-500"}`}
              />
              {product.stock > 0
